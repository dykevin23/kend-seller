...
import type { Route } from "./+types/admin-inquiry-list-page";
import { makeSSRClient } from "~/supa-client";
import { getAdminGeneralInquiries } from "../queries";
...
export const loader = async ({ request }: Route.LoaderArgs) => {
  ...
  const inquiries = await getAdminGeneralInquiries(client, {
    status,
    category,
  });
          ...
          {inquiries.length > 0 ? (
            inquiries.map((item) => (
              <TableRow
                ...
                onClick={() => navigate(`/system/inquiries/${item.id}`)}
              >
                ...
                <TableCell className="max-w-[320px] truncate py-3">
                  {item.title}
                </TableCell>
                <TableCell className="py-3">{item.writer_nickname}</TableCell>
