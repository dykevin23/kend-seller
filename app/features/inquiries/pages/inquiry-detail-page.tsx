...
import { getSellerInquiryDetail } from "../queries";
import { answerInquiry } from "../mutations";
...
export const action = async ({ request, params }: Route.ActionArgs) => {
  ...
  return await answerInquiry(client, {
...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
  ...
  const inquiry = await getSellerInquiryDetail(client, {
    sellerId: seller.id,
    inquiryId: params.inquiryId,
  });
  return { inquiry };
    ...
    if (fetcher.state === "idle" && wasSubmittingRef.current) {
      ...
      if (fetcher.data?.success) {
        alert({
          ...
          primaryButton: {
            label: "확인",
            onClick: () => navigate("/orders/inquiries"),
          },
        });
      }
    }
      ...
      <Content>
        ...
        <Card>
          ...
          <Button variant="outline" className="mt-4" asChild>
            <Link to="/orders/inquiries">목록으로</Link>
          </Button>
        </Card>
      </Content>
    );
    ...
    <Content className="space-y-4">
      ...
      <Card>
        ...
        <div className="mt-3">
          ...
          <Separator />
          <InfoRow label="주문번호" value={inquiry.order_number} />
          <Separator />
          <InfoRow label="상품" value={inquiry.product_name} />
          <Separator />
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => navigate("/orders/inquiries")}>
          목록으로
        </Button>
