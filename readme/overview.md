...
> 최종 업데이트: 2026-09-17 (디자인 리뉴얼 + Seller 대시보드(B-3) 테스트 통과. CS관리(B-4)·공지사항(B-5)·판매자 정산화면은 구현 완료·테스트 대기 — Phase 3 잔여는 B-6 한 건)
...
- **Phase 3 — CS관리(B-4) 구현됨(테스트 대기)** (2026-09-17): 문의 관리 화면에 기간 필터·정렬·응답률 통계 추가. 담당자 배정은 스코프 제외
- **Phase 3 — 공지사항(B-5) 구현됨(테스트 대기)** (2026-09-17): `notices` 테이블(target: ALL/SELLER/BUYER) 신설, 관리자 CRUD(`/system/notices`) + 판매자 조회화면(`/seller/notices`, 대시보드 카드 포함). kend이 같은 테이블에서 BUYER 대상만 읽으면 됨 — kend 쪽 조회화면 연결은 kend에 알려줘야 진행 가능
...
- 공지사항(Phase 3, B-5) (2026-09-17): notices 스키마+관리자 CRUD+판매자 조회화면, 구현됨·테스트 대기
...
- **공지사항(B-5) 실사용 검증**: 관리자 등록/노출토글/삭제 + 판매자 조회화면 화면상 확인 필요 (구현만 완료, 실클릭 테스트 전). 확인되면 kend에 알려줘서 kend 쪽 `/myPage/notices` 조회화면 연결 진행
...
- **판매자 정산 조회 화면 / CS관리(B-4) / 공지사항(B-5) 실사용 테스트** — 셋 다 구현은 완료, 화면 확인 전
