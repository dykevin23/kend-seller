...
> 최종 업데이트: 2026-09-16 (재고관리 화면(B-2) 테스트 통과, 다음은 Seller 대시보드(B-3))
...
- **Phase 3 — 재고관리 화면(B-2) 완료** ✅ (2026-09-16): SKU별 재고·판매상태 인라인 수정, 재고 0→판매중 전환 차단. 부수적으로 발견한 상품명 한글 자모분해(NFD) 저장 버그도 백필+재발방지 완료. 다음은 Seller 대시보드(B-3) — 나머지(CS관리/공지사항/교환처리)는 kend 인계문서(`readme/todo/phase3-kend-seller-handoff.md`) 기준 B-4~B-6
...
- **Seller 대시보드(B-3)** — 다음 착수 예정. 기간별 매출·주문건수·인기상품 등 기본 지표(`orders`/`order_items`/`delivery_items` 본인 seller_id 필터)
- (Phase 3 관리보완, 리뷰 관리·상품수정·재고관리는 완료) CS관리 운영기능(B-4), 공지사항 스키마+admin CRUD(B-5, kend 블로킹 중), 교환 처리 화면(B-6, 정책 미정으로 착수 불가) — 상세는 `readme/todo/phase3-kend-seller-handoff.md`
