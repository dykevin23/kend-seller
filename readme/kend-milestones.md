...
**블록/보류**
- 🔀 **공지사항 화면** — `notices` 테이블 자체 없음, 항상 빈 상태. **kend-seller가 스키마+admin CRUD 화면을 먼저 만들고, kend은 조회 화면만 담당**(문의하기 P2.5-4와 같은 소유권 원칙) — kend-seller 작업 전까지 착수 불가
...
- ⏸ **"현재 위치로 주소 찾기" 버튼 미동작** — kend-native 위치 권한/좌표 전달 브릿지 선행 필요, kend 단독 해결 불가
- ⚠️ **교환(exchange, 구 P2.5-3 하위 항목)** — **이 목록의 다른 항목과 달리 드롭 불가, 구매자 대상 핵심 거래 기능**(반품과 한 가족). 개발이 밀린 게 아니라 정책(옵션재고 없을때 처리, 배송비 부담주체, 횟수제한 등)이 안 정해져서 착수 불가 — 정책 결정되는 대로 최우선 착수, Phase 3 다른 항목과 우선순위 동일시 금지
